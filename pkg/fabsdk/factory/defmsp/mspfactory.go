...
import (
	...
	kvs "github.com/aiguo186/fabric-sdk-go-gm/pkg/fab/keyvaluestore"
	"github.com/aiguo186/fabric-sdk-go-gm/pkg/fabsdk/provider/msppvdr"
...
func (f *ProviderFactory) CreateUserStore(config msp.IdentityConfig) (msp.UserStore, error) {
	...
	//fmt.Println("wcx Debug: stateStorePath=",stateStorePath)

	var userStore msp.UserStore

	if stateStorePath == "" {
		userStore = mspimpl.NewMemoryUserStore()
		//fmt.Println("wcx Debug: userStore=",userStore)
		...
		if err != nil {
			...
			//fmt.Println("wcx Debug: creating a user store succeed")
		}
	}
